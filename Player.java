...
class Laser {
    ...
    int speed = 15;
    boolean active = true;
    public Laser(int startX, int startY) { this.x = startX; this.y = startY; }
    public void move() { x += speed; if (x < 0) active = false; }
    public void draw(Graphics g) { g.setColor(Color.RED); g.fillRect(x, y, width, height); }
    public Rectangle getBounds() { return new Rectangle(x, y, width, height); }
}
