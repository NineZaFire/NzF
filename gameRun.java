...
class gameRun extends JPanel {
    ...
    private int stageDuration = 15000;
    ...
    public gameRun() {
        ...
        timerBG = new Timer(16, e -> { 
                moveBackground(); // print background and move from right to left 
        }); 
    ...
    private void spawnBoss() {
        ...
        b.setbossRef(player, this, bossLasers);
        enemies.add(b);
    }
    ...
    private void checkCollisions() {
        ...
        for (Enemy e : enemies) {
            if (e instanceof BossEnemy boss && !boss.isAlive() && bossAlive) {
                ...
                enemyDMG += 1;
                bossDMG += 3;
                BossEnemy.healthBuff += Math.pow(8,BossEnemy.healthBuff);
                enemies.remove(boss);
                break;
            }
        }
        ...
        if (!player.Isalive() && !gameOverScheduled) {
            gameOverScheduled = true;

            // stop all timers immediately (including collision timer) to prevent repeated scheduling
            timerBG.stop();
            timerE.stop();
            timerPL.stop();
            timerCollision.stop();
            ...
            // trigger explosion visual (if you still want to show it before switching)
    ...
    private void gameOver() {
        SwingUtilities.invokeLater(() -> {
            Starshipshooter.gamerun.getContentPane().removeAll();
            Starshipshooter.gamerun.add(new GameOver());
            Starshipshooter.gamerun.revalidate();
            Starshipshooter.gamerun.repaint();
        });
    }
